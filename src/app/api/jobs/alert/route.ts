...
// ── GET /api/jobs/alert ───────────────────────────────────────────────────────
// Returns current alert subscription status for the user
...
// ── POST /api/jobs/alert ──────────────────────────────────────────────────────
// Body: { action: 'subscribe'|'unsubscribe', cvMeta?: JobsRequest }
// subscribe:   upsert job_alerts row + send confirmation email
// unsubscribe: set subscribed=false
...
// ── GET /api/jobs/alert/unsubscribe?token=... ─────────────────────────────────
// 1-click unsubscribe from email link (no auth required)
// Note: this is handled by /api/jobs/alert/unsubscribe/route.ts (separate file)

// ── Email templates ───────────────────────────────────────────────────────────

function buildConfirmationEmail({ email, domain, level, unsubscribeToken }: {
    ...
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#0E0D1C;border:0.5px solid rgba(255,255,255,0.07);border-radius:12px;overflow:hidden">
        ...
        <tr><td style="padding:28px 32px">
          <h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#ECEAF8;letter-spacing:-0.5px">
...
export function buildJobAlertEmail({ email, domain, level, jobs, unsubscribeToken }: {
  ...
  const jobCards = jobs.slice(0, 5).map(job => {
    ...
    const strengthsList = job.strengths.map(s =>
      `<tr><td style="padding:2px 0;font-size:12px;color:#8884AA;line-height:1.5">
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0">
        ...
        <tr><td style="padding-bottom:20px">
          ...
          <p style="margin:0;font-size:14px;color:#8884AA">
            Matched to your ${level} profile · sorted by fit score
          </p>
        </td></tr>
