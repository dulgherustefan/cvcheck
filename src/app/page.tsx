...
function TipCard({ tip, index, locked, onUnlock }: {
  ...
  return (
    <div className={`${styles.tipCard} ${open ? styles.tipCardOpen : ''}`}>
      <button className={styles.tipHeader} onClick={() => setOpen(!open)}>
        ...
        <svg className={`${styles.tipChevron} ${open ? styles.tipChevronOpen : ''}`}
              ...
              <div style={{ display: 'flex', background: 'var(--bg-subtle)', borderRadius: 'var(--radius-md)', padding: 3, gap: 2 }}>
                {(['login', 'register'] as const).map(m => (
                  <button key={m} onClick={() => { setAuthMode(m); setAuthError('') }} style={{
                    ...
                    background: authMode === m ? 'var(--bg-elevated)' : 'transparent',
    ...
    <div className={styles.page}>
          ...
          <div className={styles.headerRight}>
            {!authLoading && (user ? (
              ...
              <div className={styles.headerAuthBtns}>
                ...
                <button className={styles.upgradeHeaderBtn} onClick={() => setShowUpgradeModal(true)}>
                  Get Pro — €2
                </button>
              </div>
            ))}
      ...
      {showAccountModal && user && (
        <AccountModal
          ...
          userId={user.id}
          userEmail={user.email ?? ''}
          onUpgrade={() => { setShowAccountModal(false); setShowPlansModal(true) }}
          onSignOut={() => { setShowAccountModal(false); signOut() }}
        />
      )}
      {showPlansModal && (
        <PlansModal
          ...
          onBuy={() => { setShowPlansModal(false); setShowUpgradeModal(true) }}
        />
      )}
            ...
            <div className={styles.inputCard}>
              ...
              {mode === 'url' && (
                <div className={styles.urlInputWrapper}>
                  <svg className={styles.urlIcon} width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
              ...
              <button className={styles.submitBtn} onClick={submit}
                disabled={appState === 'loading' || (mode === 'url' ? !url.trim() : !file)}>
                Roast it
                <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
